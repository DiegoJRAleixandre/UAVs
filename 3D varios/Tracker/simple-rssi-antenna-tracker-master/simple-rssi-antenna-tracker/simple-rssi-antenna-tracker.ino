...
#define RSSI_OFFSET_LEFT    5

/*
   MIN and MAX RSSI values corresponding to 0% and 100% from receiver
   See serial monitor
*/
#define RSSI_MAX            900 //723 max
#define RSSI_MIN            302 //120 min
...
//  }
  ...
  */
  ...
  if (debug) {
    Serial.print("RSSI%: ");
    Serial.print(map(avgLeft, RSSI_MIN, RSSI_MAX, 0, 99));
    Serial.print(", ");
    Serial.print(map(avgRight, RSSI_MIN, RSSI_MAX, 0, 99));
...
void measureRSSI() {
  ...
  rssi_left_array[LAST] = 1024-analogRead(LEFT_RSSI_PIN);
  rssi_right_array[LAST] = 1024-analogRead(RIGHT_RSSI_PIN);
}
